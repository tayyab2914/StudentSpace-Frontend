...
export default function Instructor() {
  ...
  const fetch_reviews = async () => {
    try {
      const response = await API_GET_FACULTY_REVIEWS(setShowSpinner, slug);
        ...
        <>
          <Breadcrumb 
            ...
            items={[
              ...
              { title: <a onClick={() => router.push(`/department/${getFacultyNameByDepNo(facultyInfo?.department)}`)}>{getFacultyNameByDepNo(facultyInfo?.department).toUpperCase()}</a>},
              ...
              <div className="row">
                <div className="col-12 col-xl-4 text-center p-0">
                  ...
                  <div className="text-center px-xl-5">
                    <Image 
                      src={facultyInfo?.image_url || '/placeholder.jpg'} 
                      alt={facultyInfo?.name || 'Faculty'} 
                      width={200} 
                      height={200}
